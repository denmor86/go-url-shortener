...
def check_test_app_exist(test_bin_path):
     if not os.path.isfile(test_bin_path):
          raise FileNotFoundError(f'Исполняемый файл тестов {APP_NAME} не найден')
     
def main():
    parser = argparse.ArgumentParser(description='Запуск тестов инкрементов')
    parser.add_argument('--test_bin_path', type=str, default=APP_NAME, help='Путь к приложению для тестирования')
    parser.add_argument('--increments', type=int, default=9, help='Количество проверяемых инкрементов')
    parser.add_argument('--bin_path', type=str, default=DEFAULT_BIN_PATH, help='Путь к приложению shortener')
    parser.add_argument('--src_path', type=str, default=DEFAULT_SRC_PATH, help='Путь к исходникам приложения')
    parser.add_argument('--port', type=int, default=DEFAULT_PORT, help='Порт')
    parser.add_argument('--file_storage_path', type=str, default=DEFAULT_CACHE_FILE_PATH, help='Путь к файлу кэша URLs')
        ...
        for i in range(args.increments):
            ...
            try:
                process = subprocess.Popen([test_bin_path, test_verbose, current_test, bin_path, src_path, port, file_storage_path],
                                           ...
                                           stderr=sys.stderr,
                                           text=True)
                print(f"Тест запущен со следующими параметрами: {process.args}")
